...
export const createEmprendedor = async (req, res) => {
    try {
        const { nombre, mail, password, direccion, telefono, actividad, msg_description} = req.body;
...
export const getEmprendedorMailPass = async (req, res) => {
    const emprendedor = await Emprendedor.findOne({email: req.body.email}).select('+password');
    ...
    const validPassword = await emprendedor.comparePassword(req.body.password, emprendedor.password)
    ...
    const token = jwt.sign({ id: emprendedor._id }, config.secret, { expiresIn: 60 * 60 * 2});
    res.status(200).json({ 
        auth: true, 
        token, 
        emprendedor
...
export const updateEmprendedor = async (req, res) => {
    ...
    try {
        const registro = await Emprendedor.findByIdAndUpdate(
            _id,
            body,
            {new: true}
        );
